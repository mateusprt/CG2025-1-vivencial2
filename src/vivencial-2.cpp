...
vec3 cameraFront = vec3(0.0f, 0.0, -1.0f);
...
struct Material
{
	vec3 Ka;						// cor ambiente
	vec3 Ks;						// cor especular
	float Kd;						// cor difusa
	float Ns;						// shininess
...
int main()
{
	...
	objects[0].offsetX = -0.5f;
	objects[1].offsetX = 0.5f;
	...
	// Matriz de view
	mat4 view = lookAt(cameraPos, vec3(0.0f, 0.0f, 0.0f), cameraUp);
	glUniformMatrix4fv(glGetUniformLocation(shader.ID, "view"), 1, GL_FALSE, value_ptr(view));

	// Matriz de projeção
	mat4 projection = perspective(radians(39.6f), (float)WIDTH / HEIGHT, 0.1f, 100.0f);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (size_t i = 0; i < NUM_OBJECTS; i++)
		{
			...
			if (rotateX)
				objects[i].model = rotate(objects[i].model, angle, vec3(1.0f, 0.0f, 0.0f));
			if (rotateY)
				objects[i].model = rotate(objects[i].model, angle, vec3(0.0f, 1.0f, 0.0f));
			if (rotateZ)
				objects[i].model = rotate(objects[i].model, angle, vec3(0.0f, 0.0f, 1.0f));

			glUniformMatrix4fv(modelLoc, 1, GL_FALSE, value_ptr(objects[i].model));
...
void key_callback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	if ((key == GLFW_KEY_W || key == GLFW_KEY_UP) && action == GLFW_PRESS)
		objects[selectedObject].offsetY += 0.05f;
	if ((key == GLFW_KEY_S || key == GLFW_KEY_DOWN) && action == GLFW_PRESS)
		objects[selectedObject].offsetY -= 0.05f;

	if ((key == GLFW_KEY_A || key == GLFW_KEY_LEFT) && action == GLFW_PRESS)
		objects[selectedObject].offsetX -= 0.05f;
	if ((key == GLFW_KEY_D || key == GLFW_KEY_RIGHT) && action == GLFW_PRESS)
		objects[selectedObject].offsetX += 0.05f;

	if ((key == GLFW_KEY_Q || key == GLFW_KEY_LEFT) && action == GLFW_PRESS)
		objects[selectedObject].offsetZ -= 0.05f;
